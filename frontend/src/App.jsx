...
function App() {
  ...
  const scanSingle = async () => {
    ...
    try {
      ...
      setResults([{ domain: domain.trim(), error: 'Could not reach the scanner API. Is app.py running?' }])
    }
    setLoading(false)
  }
  ...
  const runBatch = async (domains) => {
    ...
    try {
      ...
      setResults([{ domain: 'batch', error: 'Could not reach the scanner API. Is app.py running?' }])
    }
    setLoading(false)
  }
  ...
  const getBadge = (result) => {
    ...
    if (result.pqc_readiness === 'NOT PQC-Ready') return <span className="badge not-ready">NOT READY</span>
  ...
  return (
    <div className="app">
      <div className="hero">
        <h1>PQC <span>Readiness</span> Scanner</h1>
        <p>Check whether a domain's TLS certificate can survive Q-Day</p>
        <div className="qday">Est. Q-Day: ~2030–2035 · migrate before harvest-now-decrypt-later catches up</div> 
        <div className="hero-warning">⚠️ Results reflect certificate signature strength only — not full TLS handshake security. A "NOT READY" cert doesn't mean the site is insecure today.</div>
      </div>
      ...
      <div className="input-panel">
        {mode === 'single' && (
          <>
            ...
            />
            <button className="scan-btn" onClick={scanSingle} disabled={loading}>
          ...
          <>
            ...
            {batchInputType === 'manual' && (
              <>
                ...
                />
                <button className="scan-btn" onClick={scanBatchManual} disabled={loading}>
            ...
            {batchInputType === 'file' && (
              <>
                ...
                />
                <button className="scan-btn" onClick={scanBatchFile} disabled={loading || !file}>
        ...
        <div className="result-card" key={i}>
          ...
          {result.error ? (
            ...
            <div className="result-detail">
              Signature: <span>{result.signature_algorithm}</span><br />
              Public Key: <span>{result.public_key_type}</span><br />
